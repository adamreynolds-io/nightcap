## 1. Node Command
- [ ] 1.1 Implement `nightcap node` task
- [ ] 1.2 Generate docker-compose.yml for Midnight stack
- [ ] 1.3 Pull required images with progress display
...
- [ ] 2.1 Configure `midnightntwrk/midnight-node` container (ports 9933, 9944)
- [ ] 2.2 Configure `midnightnetwork/proof-server` container (port 6300)
- [ ] 2.3 Configure `midnightntwrk/indexer-standalone` container (port 8080)
- [ ] 2.4 Configure `midnightntwrk/wallet-indexer` container
- [ ] 2.5 Set up shared Docker network for inter-container communication
- [ ] 2.6 Configure volume mounts for persistent data

## 3. Subcommands
- [ ] 3.1 Implement `nightcap node stop` to stop stack
- [ ] 3.2 Implement `nightcap node logs [service]` to tail logs
- [ ] 3.3 Implement `nightcap node status` to show running services
...
## 7. Configuration
- [ ] 7.1 Add `networks.local` config section for Docker settings
- [ ] 7.2 Support custom image tags/versions
- [ ] 7.3 Allow port customization to avoid conflicts
- [ ] 7.4 Support resource limits (CPU, memory)

## 8. Testing
- [ ] 8.1 Integration tests for stack startup/shutdown
